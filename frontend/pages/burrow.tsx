...
const Burrow: NextPage = () => {
    ...
    const [burrowID, setBurrowID] = useState(0);

    const router = useRouter();

    useEffect(() => {
        const fetchListData = async () => {
          const res = await fetch("http://127.0.0.1:4523/mock/435762/burrows/1", {
            method: 'GET',
          });
          if (res.status === 401) {
            message.info('请先登录！');
          ...
          <div className="site-layout-background" style={{ padding: 24, minHeight: 380 }}>
            ...
            <Card>
                ...
                <List
                    ...
                    <List.Item
                        style={{background:index % 2===0?"#f1f4f8":"#FFFFFF"}}
                        key={item.title}
                        actions={[
                        <IconText icon={StarOutlined} text={item.follow} key="list-vertical-star-o" />,
